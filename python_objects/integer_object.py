...
# Computers can not store integers directly
	# only can store binary numbers(0, 1) to represent integer objects
		# eg. to store 5 computers need to represent it using a BASE-2 number: 5 = 1 x 2**2 + 0 x 2**1 + 1 x 2**0 (3 bits)
...
#Because the number zero doesn’t have a sign, computers can squeeze out an extra number. so, 8 bits can store all integers from -128 to 127. 8-bits = [-2**7, 2**7 – 1]
...
# How Python represents integers?:
    ...
    # integers are objects that Python needs an extra fixed number of bytes as an overhead for each integer.(Python integer overhead)
		# though int has FIXED memory capacity
