# None
	...
	# not zero (0, 0.0, …) not False, Not empty string('')
...
# What is the differnt between del and None Python?
...
# It’s a good practice to use the "is" or "is not" to compare a value with None. (id memory => identity operator is)
	# The reason is that the user-defined objects may change the equality operator’s behavior by overriding the __eq__()
	
	
class Apple:

    def __eq__(self, other):        # object self == anything even None
        return True
